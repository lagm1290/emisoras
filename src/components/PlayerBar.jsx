...
export default function PlayerBar() {
  ...
  return (
    <>
      <div
        className={`fixed bottom-0 left-0 right-0 bg-[#141414]/95 backdrop-blur-md border-t border-white/10 flex items-center justify-between px-4 md:px-8 py-3 z-50 transition-transform duration-300 ${
          station ? 'translate-y-0' : 'translate-y-full'
        }`}
      >
        {/* Info */}
        <div className="flex items-center gap-3 min-w-0 w-1/3">
          <div className="w-12 h-12 rounded bg-zinc-800 flex items-center justify-center text-xl shrink-0">
            &#127897;
          </div>
          <div className="min-w-0 hidden md:block">
            <h4 className="text-sm font-semibold truncate">{station?.titulo || ''}</h4>
            <p className="text-xs text-gray-400 truncate">{station ? `${station.cadena} - ${station.ciudad}` : ''}</p>
          </div>
          <button
            onClick={() => station && toggleFavorite(currentIndex)}
            className="text-gray-400 hover:text-red-500 transition md:ml-2"
          >
        ...
        <div className="flex flex-col items-center gap-1 flex-1 max-w-md">
          <div className="flex items-center gap-5">
            <button onClick={prevStation} className="text-gray-400 hover:text-white transition">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg>
            </button>
            <button
              onClick={togglePlay}
              className="w-9 h-9 rounded-full bg-white text-black flex items-center justify-center hover:scale-110 transition"
            >
              {isPlaying ? (
            ...
            <button onClick={nextStation} className="text-gray-400 hover:text-white transition">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg>
            </button>
          </div>
          <div className="w-full flex items-center gap-2 text-[11px] text-gray-500">
            <span className="w-8 text-right">{elapsedTime}</span>
        ...
        <div className="flex items-center gap-3 w-1/3 justify-end">
          ...
          <div
            className="w-20 h-1 bg-gray-700 rounded-full relative cursor-pointer hidden md:block"
            ref={volSliderRef}
